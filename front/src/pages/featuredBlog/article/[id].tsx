...
const featuredBlogPage: NextPage<Props> = ({ posts, featuredArticle }) => {
  return (
    <>
      <PageSEO
        path={`featuredBlog/article/${posts.id}`}
        title={`${posts.title} - HAL Blog`}
        description={posts.summary}
        ogImageUrl={posts.media.url}
      />
      <Breadcrumb title={posts.title} category="注目記事" />
      <ContentWrapper>
        <ArticlesContainer>
          <Article>
            <TagAndDateContainer>
...
const ArticleContent = styled.section`
  ...
  h1 {
    font-size: ${({ theme }) => theme.fontSizes.large};
    color: ${({ theme }) => theme.colors.primary.black};
    border-bottom: 2px solid ${({ theme }) => theme.colors.primary.red};
    padding-bottom: 8px;
    ${mediaQuery.sm`
      font-size: ${({ theme }) => theme.fontSizes.large};
    `}
