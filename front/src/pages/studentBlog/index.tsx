...
const Page: NextPage<Props> = ({ featuredArticle, studentArticle }) => {
  return (
    <>
      <PageSEO
        path="featuredBlog"
      ...
      />
      <ContentWrapper>
        <BlogHeading>生徒ブログ</BlogHeading>
        <ArticlesContainer>
          <LeftAreaContainer>
            <CardContainer>
              {studentArticle.map((item) => (
                <ArticleCard
                  key={item.id}
                  id={item.id}
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const studentArticle = featuredArticle.filter(
    (item) => item.category.slug === "student"
  );

  return {
    props: { featuredArticle, studentArticle },
  };
};
