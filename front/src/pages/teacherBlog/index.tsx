...
const Page: NextPage<Props> = ({ featuredArticle, teacherArticle }) => {
  return (
    <>
      <PageSEO
        path="featuredBlog"
      ...
      />
      <ContentWrapper>
        <BlogHeading>教師ブログ</BlogHeading>
        <ArticlesContainer>
          <LeftAreaContainer>
            <CardContainer>
              {teacherArticle.map((item) => (
                <ArticleCard
                  key={item.id}
                  id={item.id}
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const teacherArticle = featuredArticle.filter(
    (item) => item.category.slug === "teacher"
  );

  return {
    props: { featuredArticle, teacherArticle },
  };
};
